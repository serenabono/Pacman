...
class QLearningAgent:
	...
	def __init__(self, exploration_strategy="BOLTZMANN", T=None, epsilon=None, on_policy=False, initialization_value=0, gamma=0.9, alpha=0.05, is_train=False, load_existing_agent=False):
		...
		if is_train:
			...
			###### Boltzmann exploration hyperparameters 
				...
				if epsilon == None:
					raise("Please specify Hyperparameter epsilon for e_greedy exploration")
				else:
		...
		if (not is_train) or (load_existing_agent):
			try:
				...
				raise("Object pickle file not present. Please train and write the agent to disk first.")
	...
	def sample_weighted_from_actions(self, actions_dict):
 	...
 	def sample_from_current_actions_boltzmann(self):
 		...
 		exploration_action = self.sample_weighted_from_actions(q_value_to_prob_map)
 		max_action = np.random.choice(max_actions_arr)
	...
	def on_policy_update(self, reward):
		if self.current_state_type == "initial":
			...
			self.q_values[self.previous_state][self.previous_action] +=  self.ALPHA*(reward + (self.GAMMA*self.q_values[self.current_state][action]) - self.q_values[self.previous_state][self.previous_action])
		elif self.current_state_type == "terminal":
			self.q_values[self.previous_state][self.previous_action] +=  self.ALPHA*(reward + (self.GAMMA*0) - self.q_values[self.previous_state][self.previous_action])
	...
	def off_policy_update(self, reward):
		if self.current_state_type == "initial":
			...
			max_action_temp = None
	 		...
	 		max_action = np.random.choice(max_actions_arr)
	 		self.q_values[self.previous_state][self.previous_action] +=  self.ALPHA*(reward + (self.GAMMA*self.q_values[self.current_state][max_action]) - self.q_values[self.previous_state][self.previous_action])
